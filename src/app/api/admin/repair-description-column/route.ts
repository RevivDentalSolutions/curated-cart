...
function hasOneTimeConfirmation(request: NextRequest) {
  const configuredConfirmation = process.env.DATABASE_REPAIR_CONFIRMATION;
  const providedConfirmation = request.nextUrl.searchParams.get('oneTimeConfirmation') || request.headers.get('x-repair-confirmation');
  return Boolean(configuredConfirmation && providedConfirmation && providedConfirmation === configuredConfirmation);
}
...
export async function GET(request: NextRequest) {
  if (!hasRepairToken(request) || !hasOneTimeConfirmation(request)) {
    return NextResponse.json({ success: false, error: 'Valid DATABASE_REPAIR_TOKEN and DATABASE_REPAIR_CONFIRMATION required' }, { status: 401 });
  }
  ...
  try {
    ...
    if (existedBefore) {
      return NextResponse.json({
        ...
        error: 'Product.description already exists. Repair route is disabled after successful repair.',
        nextSteps: ['Remove DATABASE_REPAIR_TOKEN and DATABASE_REPAIR_CONFIRMATION from Vercel.'],
      }, { status: 409 });
    }
    ...
    return NextResponse.json({
      ...
      nextSteps: [
        ...
        'Remove DATABASE_REPAIR_TOKEN and DATABASE_REPAIR_CONFIRMATION after this one-time repair is complete.',
      ],
    });
  } catch (error: unknown) {
