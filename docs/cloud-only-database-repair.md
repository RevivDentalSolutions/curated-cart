...
Then add a strong one-time secret in Vercel Production environment variables:

```text
DATABASE_REPAIR_TOKEN=<long random secret>
```

Remove `DATABASE_REPAIR_TOKEN` after the repair is complete. Remove `PRISMA_SKIP_MIGRATE_DEPLOY` only after migration history has been safely baselined or Vercel will return to the baseline gate.
...
```text
https://YOUR_DOMAIN.com/api/admin/repair-description-column?confirm=ADD_DESCRIPTION_COLUMN&token=YOUR_DATABASE_REPAIR_TOKEN
```
...
It does not delete data, reset the database, or mark migrations as applied.
